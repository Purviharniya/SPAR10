...
class BertEmbedding:
    ...
    def _pooled_handler(self, hidden: tf.Tensor,
        ...
        :param hidden: The hidden tf tensor to process.
    ...
    def extract_embeddings(
        ...
        if reduce_option == 'concat_last_4':
            ...
            cat_hidden_states = tf.concat(tuple(last_4), axis=-1)
            return tf.reduce_mean(cat_hidden_states, axis=1).squeeze()

        elif reduce_option == 'reduce_last_4':
            last_4 = [hidden_states[i] for i in (-1, -2, -3, -4)]
            return tf.cconcat(tuple(last_4), axis=1).mean(axis=1).squeeze()
            ...
            cat_hidden_states = tf.concat(tuple(last_states), axis=-1)
            return tf.reduce_mean(cat_hidden_states, axis=1).squeeze()

        last_states = [hidden_states[i] for i in hidden]
        hidden_s = tf.concat(tuple(last_states), axis=1)
